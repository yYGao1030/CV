...
class FruitDetectionSystem:
    ...
    def _load_model(self):
        """安全加载模型."""
        load_attempts = [self._load_from_local_repo, self._load_via_torch_hub, self._load_directly]
    ...
    def _load_from_local_repo(self):
        ...
        if local_repo.exists():
            return torch.hub.load(str(local_repo), "custom", path=str(self.weights_path), source="local")
    ...
    def detect(self, img_input, conf_threshold=0.3, iou_threshold=0.45, visualize=True):
        ...
        counts, output_img = self._process_results(results, img.copy() if visualize else None)
    ...
    def _process_results(self, results, output_img=None):
        ...
        if output_img is not None:
            ...
            for _, det in detections.iterrows():
                ...
                draw.rectangle([x1, y1, x2, y2], outline=class_info["color"], width=2)

                # 绘制中文标签
                label = f"{class_info['display']} {det['confidence']:.2f}"
                draw.text((x1, y1 - 25), label, fill=class_info["color"], font=font)
